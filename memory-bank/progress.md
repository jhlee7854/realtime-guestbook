...
3. 실제 Supabase 프로젝트에서 `supabase/schema.sql`을 적용한다.
4. Realtime publication에 `guestbook_posts`, `comments` 테이블이 포함되었는지 확인한다.
5. 두 브라우저 창으로 방명록 insert, 댓글 insert 실시간 갱신을 수동 검증한다.
